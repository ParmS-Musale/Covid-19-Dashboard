...
export function LineChartComponent({ covidData, dateRange }) {
  ...
  useEffect(() => {
    if (!covidData || !covidData.timeline) return
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <LineChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 10 }}>
        ...
        <Legend />
        <Line type="monotone" dataKey="cases" stroke="#8884d8" strokeWidth={2} dot={{ r: 1 }} activeDot={{ r: 5 }} />
        <Line
          type="monotone"
          dataKey="recovered"
          stroke="#82ca9d"
          ...
          activeDot={{ r: 5 }}
        />
        <Line type="monotone" dataKey="deaths" stroke="#ff7c7c" strokeWidth={2} dot={{ r: 1 }} activeDot={{ r: 5 }} />
      </LineChart>
    </ResponsiveContainer>
  )
}
